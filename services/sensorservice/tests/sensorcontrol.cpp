...
// time tracking
static msecs_t beforePollMs = 0UL, lastMs = 0UL;
    ...
    while ((n = q->read(buffer, bufferSize)) > 0) {
        for (int ii = 0; ii < n; ii++) {
            ...
            */
            ...
            /* bail out when a meta event is received (indicated by the version
               field being 0). For now only FLUSH_COMPLETE meta event are 
               supported */
            if(buffer[ii].version == 0 && buffer[ii].sensor == 0) {
                printf("INFO Flush complete, type:%d, sensor:%d, what:%d\n",
                       buffer[ii].type,
